...
게임의 초기 배치 자체가 완료 불가능한 상태입니다. 어떤 이동 순서를 선택하더라도 승리할 수 없습니다.

**특징:**
- 게임 시작(Shuffle) 직후부터 결정됨
- 플레이어의 선택과 무관
- 카드 배치의 구조적 문제 (순환 의존성, 필수 카드 블로킹 등)
- 탐지 시점: 게임 시작 시 또는 언제든지
...
### 2. **Unwinnable State** (현재 상태에서 승리 불가능)
게임은 본질적으로 해결 가능했지만, 플레이어의 잘못된 선택으로 인해 현재 상태에서는 더 이상 진행할 수 없는 경우입니다.
...
결과: 필수 카드가 킹 밑에 갇힘
→ **Unwinnable State** (하지만 처음부터 Unsolvable은 아니었음)
```
